...
const NationalNewsDetail = props => {
   useEffect(() => {
  ...
  let { getSportNewsListData, getTopnewsdata, getNationalNewsList, getInternationalNewsList_Data } = users;
  const OnClick = () => {
      ...
      <View style={{ borderBottomWidth: 2, borderBottomColor: 'lightgrey', flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 15, paddingHorizontal: 5, marginHorizontal: 10 }} >
        ...
        <ScrollView>
          <View style={{ marginBottom: 20, }}>
            ...
            <>
              {getInternationalNewsList_Data && getInternationalNewsList_Data.length > 0 ? getInternationalNewsList_Data.map((element, index) => (
                  <>
                    <TouchableOpacity onPress={() => navigation.navigate('InternationalNewsDetails', { id: element.id })}>
                      ...
                      <View style={{ height: 30, flexDirection: 'row', }}>
                        <View style={{ width: '50%', height: '100%' }}>
                          <Text style={{ color: 'black', left: 10, fontSize: 14, fontWeight: '700' }}>राजस्थान</Text>
                        </View>
...
export default connect(mapStateToProps)(NationalNewsDetail);
