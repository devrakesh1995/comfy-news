...
const DBorijnal = props => {
  const dispatch = useDispatch();

  const { users, navigation } = props;
  let { getDBorijnalNewsListdata, } = users;
      ...
      <View style={[styles.child, { backgroundColor: '#fff' }]}>
        <View style={{ width: '100%', backgroundColor: '#fff' }}>
          ...
          <ScrollView>
            <View>
              {
                getDBorijnalNewsListdata && getDBorijnalNewsListdata.length > 0 ? getDBorijnalNewsListdata.map((element, index) => (
                  <>
                    <>{index == 0 ? <>
                      <TouchableOpacity onPress={() => navigation.navigate('NewsDetail', { id: element.id })}>
                        <View style={{ marginHorizontal: 15, marginVertical: 10, justifyContent: 'space-between', marginTop: 10 }}>
                          <View>
                            <Image source={{ uri: element.imageLinkUrl }} style={{ height: 200, width: '100%', borderRadius: 10, alignSelf: 'center' }} />
                          </View>
              ...
              }
              {
                ...
                ) : <View style={{ height:710,justifyContent:'center',alignItems:'center'}}>
                  <Image source={require('../../assets/new.png')} style={{height:300,width:300}}/>
                </View>
              }
            </View>
          </ScrollView>
...
export default connect(mapStateToProps)(DBorijnal);