import {
  ...
  const onShare = async (text) => {
    ...
    try {
      const result = await Share.share({
        ...
        url: 'https://www.comfynews.site/api/v1/getCategoryAndSubcategory'
      });
  ...
  const Bollywood = props => {
    ...
    let { getBollywoodNewsListdata } = users;
    // console.log("))))))))))))))))))",getBollywoodNewsListdata)
    return (
      <View>
        <View style={[styles.child, { backgroundColor: 'white' }]}>
          <View style={{ width: '100%', backgroundColor: '#fff' }}>
            ...
            <ScrollView>
              <View style={{ }}>
              {
                  getBollywoodNewsListdata && getBollywoodNewsListdata.length > 0 ? getBollywoodNewsListdata.map((element, index) => (
                    <>
                    <>{index==0?<>
                      <TouchableOpacity onPress={() => navigation.navigate('NewsDetail', { id: element.id })}>
                        <View style={{  marginHorizontal: 15, marginVertical: 10, justifyContent: 'space-between', marginTop: 10 }}>
                          <View style={{ width:320, marginTop: 15 }}>
                            <Text style={{ fontWeight: '800', color: 'black', fontSize: 14, }} numberOfLines={3}>
                              {element.title}..
                            </Text>
                          </View>
                          <View>
                            <Image source={{ uri: element.imageLinkUrl }} style={{ height: 200, width: '100%', borderRadius: 10,alignSelf:'center',marginTop:10 }} />
                          </View>
                        </View>
                      ...
                      <TouchableOpacity onPress={() => navigation.navigate('NewsDetail', { id: element.id })}>
                        <View style={{ flexDirection: 'row', marginHorizontal: 15, marginVertical: 10, justifyContent: 'space-between', marginTop: 10 }}>
                          <View style={{ width: 240, marginTop: 15 }}>
                            <Text style={{ fontWeight: '800', color: 'black', fontSize: 14, }} numberOfLines={2}>
                              {element.title}
                            </Text>
                            <HTMLView
  ...
  export default connect(mapStateToProps)(Bollywood);